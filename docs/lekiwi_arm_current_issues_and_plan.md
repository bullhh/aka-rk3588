...
也已修复。机械臂使能时先保持当前位置，HOME限速回位；夹球后先CLEAR，再以关节空间 S 曲线
...
- 扭矩使能前先把实测当前位置写成目标；任意起始姿态随后按配置限速在关节空间
  回HOME。速度由 `motion_speed_level` 统一选择；当前4档为普通轨迹50°/s、HOME
  25°/s、夹爪60°/s，启动回位仍保留安全上限。
- 完整程序重启时若夹爪反馈大于25°，先保持机械臂姿态并慢速张开ID6至100°释放
  遗留球，再慢速回HOME并重新进入CHASE，不会带球强行闭合回位。
- PRE_GRAB腕部角度自动限制在安全范围，随后用五次S曲线直接过渡到记录的最终ID4；
  不再用固定总俯仰角反算整段腕部轨迹。启动前会抽样检查轨迹和关节安全范围。
- 夹球后先回到 PRE_GRAB 高度，再以五次 S 曲线同步收折5个关节进入CARRY，不再经过
  二连杆原点附近的笛卡尔LIFT路径。
- CARRY使用手动记录的车轮启动前姿态；夹爪值不使用手动记录，仍沿用原夹持方案。
- 夹爪闭合时使用“稳定位置+闭合残差”提前识别接触并冻结目标；ID6的单独`0x20`
  只在夹爪接触、释放和残留保护恢复阶段允许，不再误报为总线通信失败。
- CARRY用40个20 Hz周期完成，随后保持10个周期；车轮只会在反馈收敛且保持结束后启动。
- 抓取点已从 `(0.09,-0.08)` 调整到 `(0.12,-0.06)`，减小肩肘折叠和腕部负角度。
- MOVE_TO 和腕部阶段现在等待实际关节反馈收敛；小于8°的非夹爪残差直接发送最终目标，
