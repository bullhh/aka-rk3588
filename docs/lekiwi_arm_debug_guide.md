...
当前动作算法参考 Desktop-Wanderer。第一阶段重点是让完整闭环中的机械臂更新频率
接近独立 `ik-pick` 测试，而不是重新设计轨迹。

1. 运行 `test-new-arm auto ik-pick`，录像并记录总时长。
2. 在完整流程中录像同一机械臂阶段。
3. 若独立测试流畅而完整流程卡顿，优先检查 `PICK_BALL/PUT_BALL` 是否仍随
   2-3 FPS 的视觉循环每帧只调用一次 `tick()`。
4. 第一阶段先使用 20-25 Hz，稳定后再尝试 50 Hz。
5. `GAP` 应固定在约 300-400 ms，不能乘上视觉帧间隔。
