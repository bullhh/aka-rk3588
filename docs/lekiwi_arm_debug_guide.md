...
| 机械臂和底盘整体太慢 | `motion_speed_level` | 从1～4逐级增加 |

不要用增大`tolerance`来掩盖夹不到球；容差越大，停车点的前后离散越大。先让同一位置
连续停车3～5次基本一致，再调整机械臂。

速度只调整 `motion_speed_level`：1调试、2稳定、3快速、4极速。程序内部保持放球下降
慢于高位移动、近距离底盘慢于远距离底盘，并自动选择等待时间。修改后先运行
`config-check`；无效等级会直接报错，程序不会执行动作。

近球阶段不会因中心偏差刚超过阈值就停下原地转向：程序会保持前进，并用左右轮差速
沿弧线修正；只有已经到达停车距离但仍未对正时才原地微调。进入和退出精调使用不同
阈值，可避免检测框在边界附近抖动造成“动作、停、动作”。
