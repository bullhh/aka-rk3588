...
当前程序不是一组彼此独立的演示脚本，而是同一个 `build/tennis` 二进制中的完整控制
栈：

| 模块 | 当前实现 |
| --- | --- |
| 摄像头 | libuvc采集640×480 MJPEG，回调线程只保留最新一帧 |
| 网球识别 | RKNN YOLOv8；Linux默认使用3个NPU核，StarryOS默认使用NPU核0 |
| 桶识别 | 全分辨率HSV红色区域检测，不依赖网球模型 |
| 底盘 | ID7、ID8、ID9三轮全向底盘，支持平移、旋转和精细对正 |
| 舵机通信 | `auto`先尝试userspace libusb CDC，Linux无权限时回退TTY |
| 机械臂 | ID1～ID6反馈闭环、校准角度、50ms控制周期、限速S曲线和动作安全校验 |
| 任务控制 | 追球、抓球复核/重试、找桶、放球、撤离组成持续状态机 |

机械臂执行抓球或放球时，主循环本来就不会消费摄像头图像。当前版本会暂时停止UVC
数据流，动作完成后在原设备句柄上恢复，从而避免摄像头和Feetech CDC竞争同一USB
路径。它不会重新枚举摄像头，也不会再次执行20帧预热。

### 1.2 当前版本的主要改进

| 早期实机问题 | 当前处理 | 效果 |
| --- | --- | --- |
| Starry执行`scan`后下一进程读不到舵机 | CDC事务消费写ACK、丢弃过期状态包，并正确释放接口 | `scan/read`可跨进程连续运行 |
| 上电或启动闭环时机械臂突然跳回初始位 | 上扭矩前读取并保持当前位置，再限速回HOME | 任意初始位置启动更安全 |
| 夹爪中已有球时重启直接报过载 | 识别ID6接触/过载，先主动打开夹爪，再恢复HOME | 不再带球强行收臂 |
| 固定总俯仰角导致夹球姿态倾斜或中途触限 | 最终抓球姿态直接配置ID1～ID5，位置偏移只用于小范围修正 | 姿态直观、可单关节调试 |
| 夹球接触被当成舵机故障 | ID6的`0x20`或稳定大残差作为接触信号，其他舵机错误仍停机 | 能保持原有夹力且不误报失败 |
| 反馈存在数度静差时动作经常超时 | 使用符合实机精度的关节/夹爪容差；CLEAR失败时安全释放并回HOME | 负载变化时不易误报，同时保留安全退路 |
| 小车前进靠近时夹不到、后退反而能夹到 | 缩小停车尺寸容差，支持过近后退和连续稳定帧确认 | 抓球前后位置更一致 |
| 放球动作突然、机械臂碰桶 | 高位接近与最终放球姿态解耦，分段S曲线并检查完整轨迹 | 可安全越过桶沿后再下降释放 |
| 桶有角度偏差时仍继续前进 | 超出中心容差时禁止前进，低速旋转对正后再确认距离 | 放球位置更稳定 |
| Starry完整视觉流程中机械臂一卡一卡 | 绝对50ms节拍、按真实时间插值，并在动作期间暂停UVC | 实测最大控制周期由213.8ms降至50.2ms |

CPU分核曾做过实机对比：它能提高Starry视觉帧率，但机械臂最坏控制周期反而达到
430.9ms，因此当前没有固定绑核。稳定性来自减少无用USB流量，而不是单纯让动作变慢。
...
进入抓球后摄像头会暂停，日志出现
`[UvcCapture] paused before PICK_BALL` 是正常现象。机械臂完成后恢复视频并等待新帧，
再进行是否夹住球的视觉复核。
...
放球期间同样暂停UVC，恢复后才继续寻找下一颗球。StarryOS停流和恢复可能各等待数百
毫秒，这是状态切换而不是机械臂卡顿。
...
夹球准确度必须按“停车位置在前、机械臂落点在后”的顺序调整：

1. 固定摄像头、球和地面条件，先记录当前配置和一次完整日志。
2. 观察进入`BALL_READY`时的`size`和`off`，先调
   `ball_stop_size_px`、`ball_stop_tolerance_px`和`ball_center_tolerance_px`。
3. 停车重复性稳定后，固定小车和球，只运行`ik-pick`验证机械臂。
4. 优先以0.5cm步长调整`grab_forward/lateral/height_offset_cm`。
5. 只有落点正确但夹爪角度仍不合适时，才调整`grab_pitch_offset_deg`或
   `grab_id1_deg～grab_id5_deg`。
6. 空载轨迹稳定后再放置网球测试夹取，抓球稳定后再测试放球；不要同时修改停车参数、
   关节角度和夹爪力度。

更完整的初始化校验、单项命令和误差判断方法见调试手册第6节。
